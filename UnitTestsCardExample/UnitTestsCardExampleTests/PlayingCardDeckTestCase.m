...
//
//  Created by Hanguang on 11/18/14.
...
- (void)testPlayingCardDeckHasTheCorrectInitialCards {
    ...
    do {
        ...
        if ([randomCard.suit isEqualToString:@"♥️"]) [hearts addObject:randomCard];
        if ([randomCard.suit isEqualToString:@"♦️"]) [diamonds addObject:randomCard];
        if ([randomCard.suit isEqualToString:@"♠️"]) [clubs addObject:randomCard];
        if ([randomCard.suit isEqualToString:@"♣️"]) [spades addObject:randomCard];
    } while (randomCard);
