...
- (void)drawRect:(CGRect)rect
{
    ...
    for (BNRLine *line in self.finishedLines) {
        ...
        // 随机生成green和blue的颜色参数
        CGFloat randomColor = (arc4random() % 10) / 10.f;
        
        // 根据角度设置线的颜色，这里调用[UIColor colorWithRed:green:blue:alpha:]
        // 四个参数对应的取值范围都是 0.0 - 1.0，所以我的思路是 0度 - 360度 对应 0.0 - 1.0
        if (line.angle >= 0) {
            CGFloat colorParameter = line.angle / 360;
            [[UIColor colorWithRed:randomColor green:colorParameter blue:colorParameter alpha:1.0] set];
        }
        else {
            CGFloat colorParameter = (-line.angle) / 360;
            [[UIColor colorWithRed:randomColor green:colorParameter blue:colorParameter alpha:1.0] set];
    ...
    for (NSValue *key in self.linesInProgress) {
        ...
        if (line.angle >= 0) {
            CGFloat colorParameter = line.angle / 360;
            [[UIColor colorWithRed:colorParameter green:0.5 blue:1.0 alpha:1.0] set];
        }
        else {
            CGFloat colorParameter = (-line.angle) / 360;
            [[UIColor colorWithRed:colorParameter green:0.5 blue:1.0 alpha:1.0] set];
